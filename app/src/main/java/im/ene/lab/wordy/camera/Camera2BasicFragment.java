package im.ene.lab.wordy.camera;
...
import im.ene.lab.wordy.widget.AutoFitTextureView;
import java.io.File;
...
public class Camera2BasicFragment extends Fragment
      ...
      new ImageReader.OnImageAvailableListener() {
        ...
        @Override public void onImageAvailable(ImageReader reader) {
          ...
          mBackgroundHandler.post(
              new ImageSaver(reader.acquireNextImage(), mFile, new ImageSavedListener() {
                @Override public void onImageSaved(File file) {
                  // showToast("Saved: " + file);
                  if (file != null) {
                    Log.d(TAG, file.toString());
                  } else {
                    Log.i(TAG, "onCaptureCompleted: " + "file is null");
                  }
                  if (mPreviewCallback != null) {
                    mPreviewCallback.onCaptured(null, file);
                  }
                }
              }));
        }
  ...
  private void setUpCameraOutputs(int width, int height) {
    ...
    try {
      for (String cameraId : manager.getCameraIdList()) {
        ...
        Log.i(TAG, "setUpCameraOutputs: " + Arrays.toString(map.getOutputSizes(ImageFormat.JPEG)));
        ...
        if (availableSizes.size() == 0) {
          availableSizes = Arrays.asList(map.getOutputSizes(ImageFormat.JPEG));
        }

        Size largest = Collections.max(availableSizes, new CompareSizesByArea());
        Log.i(TAG, "setUpCameraOutputs: " + largest.getWidth() + " | " + largest.getHeight());
        mImageReader =
  ...
  private static class ImageSaver implements Runnable {
    ...
    private final ImageSavedListener callback;

    public ImageSaver(Image image, File file, ImageSavedListener callback) {
      this.image = image;
      this.file = file;
  ...
  private interface ImageSavedListener {
  ...
  @Override public void onAttach(Context context) {
    ...
    if (context instanceof Callback) {
      mPreviewCallback = (Callback) context;
    }
  }
  ...
  public interface Callback {
    void onCaptured(TotalCaptureResult result, File file);
  }
}
